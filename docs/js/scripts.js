...
function validateInput(e) {
  ...
  if (classes === 'not-valid'){ 
    ...
    if (e.target.parentElement.nextElementSibling.classList[0] !== 'alert') {
        ...
        insertBefore(errorDiv, e.target.parentElement.nextElementSibling);
...
togglePassword.addEventListener('click', (e) => {
    ...
    //check if the show class exist on the span element
    if(e.target.classList.contains('show')){
        //add a show class
        e.target.classList.remove('show');
        // change text for the span to hide
        ...
        //change text for the span to show
